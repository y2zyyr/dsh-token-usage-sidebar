...
- 可选的本地供应商别名分组：用户可以把多个原始供应商名称合并为一个显示分组，同时保留原始名称明细，且不改写账本。
...
供应商选项由当前时间范围的聚合数据动态生成，并采用精确匹配。账本里只有 `my-company-api` 时，界面只显示这个名称，不会凭空增加预设供应商。`scnet`、`SCNET` 和 `scnet-api` 默认保持独立，只有用户明确创建本地别名分组后才会合并。

别名分组保存在插件自有的 SQLite 数据库中，包含显示名称和原始 provider 名称列表，只影响设置页查询与展示。选择分组后，返回结果会同时提供每个原始名称的明细，便于核对合计；`usage_records` 中的原始 provider/model 值永远不会被重写。一个原始名称同一时间只能属于一个分组，避免重复归属。
...
- **本地身份映射。** 供应商别名分组与账本保存在同一个 DSH 数据目录中，只改变展示/查询分组，不会替换统计记录中的原始供应商名称。
