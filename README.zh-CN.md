...
不会被当作另一批调用再次累加。发现过程对源文件只读，重启时幂等，不会
扫描整个文件系统，也不要求用户手工填写固定路径。
...
- **旧版别名兼容。** 若旧版本曾写入供应商别名行，升级时会保留在插件自有数据库中；当前界面不再要求用户维护第二套供应商名称，也不会删除或改写统计记录。
...
当前插件版本：**v1.1.6**（npm 包 `@y2zyyr/dsh-token-usage-sidebar`；源码见 GitHub）。
