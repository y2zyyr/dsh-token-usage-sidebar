...
var DurableAggregator = class {
  ...
  insights(range) {
    ...
    if (range === "all") {
      const global = this.store.globalAggregate() ?? {};
      return { range, totalTokens: global.total_tokens ?? 0, categories: metricsOf(global), unknownTokens: global.unknown_tokens ?? 0, unknownCallCount: global.unknown_calls ?? 0, daily: dailyToDetails(this.store.dailyTotals()), models: modelsToDetails(this.store.modelTotals()) };
    }
    if (range === "7d") {
      const dates = datesEnding(now, 7);
...
function modelsToDetails(rows) {
  return rows.map((m) => ({ totalTokens: m.total_tokens, inputTokens: m.input_tokens, outputTokens: m.output_tokens, cacheReadTokens: m.cache_read_tokens, cacheWriteTokens: m.cache_write_tokens, reasoningTokens: m.reasoning_tokens, callCount: m.calls, provider: m.provider, model: m.model })).sort((a, b) => b.totalTokens - a.totalTokens);
}
function mergeDayModels(rows) {
  const map = /* @__PURE__ */ new Map();
  for (const r of rows) {
    const key = r.provider + "\0" + r.model;
    const cur = map.get(key) ?? { provider: r.provider, model: r.model, ...emptyMetrics() };
    cur.totalTokens += r.total_tokens;
    cur.inputTokens += r.input_tokens;
    cur.outputTokens += r.output_tokens;
    cur.cacheReadTokens += r.cache_read_tokens;
    cur.cacheWriteTokens += r.cache_write_tokens;
    cur.reasoningTokens += r.reasoning_tokens;
    cur.callCount += r.calls;
    map.set(key, cur);
  }
