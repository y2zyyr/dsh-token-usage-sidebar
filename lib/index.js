...
function readV1Root(v1Path) {
  let text2;
  try {
    text2 = readFileSync(v1Path, "utf8");
  } catch {
    return void 0;
  }
  try {
...
function verifyV1ToV11(dest, v1, records) {
  ...
  const sumRecords2 = records.reduce((a, r) => a + r.totalTokens, 0);
  if (sumRecords2 !== v11Lifetime) failures.push(`sum(records)=${sumRecords2} != v1.1 global=${v11Lifetime}`);
  return failures;
}
...
var SOURCE_DISCOVERY_VERSION = 2;
...
function normalizeRecord(raw, fallbackId, sourcePath, defaultDate) {
  ...
  const timestamp = finiteNumber(raw.timestamp) ?? (localDate2 !== "unclassified" ? Date.parse(`${localDate2}T12:00:00`) : 0);
  ...
  return {
    ...
    turn: finiteNumber(raw.turn) === void 0 ? identity.turn : Math.trunc(finiteNumber(raw.turn)),
    step: finiteNumber(raw.step) === void 0 ? identity.step : Math.trunc(finiteNumber(raw.step)),
    seq: finiteNumber(raw.seq) === void 0 ? 0 : Math.trunc(finiteNumber(raw.seq)),
    timestamp: timestamp !== void 0 && Number.isFinite(timestamp) ? timestamp : 0,
    localDate: localDate2,
...
function apply(ctx) {
  ...
  ctx.effect(async () => {
    ...
    try {
      ...
      sourceDiscovery = discoverTokenSources(dirname3(dbPath), {
