...
function recomputeSourceSplit(ledger) {
  ...
  for (const [id, total] of Object.entries(ledger.byId)) {
    const s = src[id];
    if (s === "live_event" || s === "other" || s === void 0) live += total;
    else {
      historical += total;
      historicalCount += 1;
    }
  }
  if (live === (ledger.liveRecordedTotal ?? 0) && historical === (ledger.historicalRecoveredTotal ?? 0) && historicalCount === (ledger.historicalRecoveredRecordCount ?? 0)) {
    return ledger;
  }
  return {
    ...ledger,
...
function normalizeLoad(raw) {
  if (raw === void 0) return { status: "none" };
  if (typeof raw === "object" && "status" in raw && (raw.status === "none" || raw.status === "ok" || raw.status === "invalid")) {
    return raw;
  }
  return { status: "ok", ledger: raw };
}
...
async function runHistoricalMigration(ledger, opts) {
  ...
  const already = (ledger.schemaVersion ?? 0) >= HISTORICAL_MIGRATION_VERSION && ledger.recovery?.recoveryStatus !== void 0 && ledger.recovery?.sourceScanStatus !== void 0 && !opts?.force;
  if (already) {
  ...
  let sourceScanStatus = "unknown";
  if (listError) {
    sourceScanStatus = "failed";
  } else if (sessionsReadFailed > 0) {
    sourceScanStatus = "partial";
  } else {
    sourceScanStatus = "complete";
  }
  ...
  } else if (sourceScanStatus === "complete" && trackingStart && earliest !== void 0) {
    const startMs = Date.parse(trackingStart + "T00:00:00");
    recoveryStatus = !Number.isNaN(startMs) && earliest >= startMs ? "complete" : "partial";
  } else {
    recoveryStatus = sessionsReadSuccessfully > 0 || newRecords.length > 0 ? "partial" : "unknown";
  }
  const recovery = {
...
var UsageAggregator = class {
  ...
  async start() {
    ...
    if (outcome.status === "ok") {
      ...
      console.warn("[dsh-token-usage-sidebar] persisted ledger failed validation; refusing to reset totals to zero. Prior history is unavailable until the store is recovered.");
    } else {
  ...
  diagnostics() {
    ...
    return {
      ...
      sourceScanStatus: this.ledger.recovery?.sourceScanStatus ?? void 0,
      recoveryStatus: this.ledger.recovery?.recoveryStatus ?? void 0,
      sessionsDiscovered: this.ledger.recovery?.sessionsDiscovered ?? void 0,
      sessionsReadSuccessfully: this.ledger.recovery?.sessionsReadSuccessfully ?? void 0,
      sessionsReadFailed: this.ledger.recovery?.sessionsReadFailed ?? void 0,
  ...
  async flush() {
    ...
    try {
      ...
      this.flushing = false;
      if (this.saveChain === next) this.saveChain = void 0;
    }
  }
  /**
...
var DomainLedgerStore = class {
  ...
  async load() {
    ...
    return { status: "ok", ledger: parsed.data };
  }
  async save(ledger) {
